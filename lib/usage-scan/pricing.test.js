...
test('costOf: claude-sonnet-5 uses sonnet rates, not the opus fallback', () => {
  assert.equal(providerOf('claude-sonnet-5'), 'claude');
  // Sonnet rates (3/15/0.3 per 1M) → distinct from the opus DEFAULT (15/75/1.5).
  // cost = 1M*3/1M + 1M*0.3/1M + 1M*15/1M = 3 + 0.3 + 15 = 18.3
  ...
  // Guard against the pre-fix regression: opus-rate fallback would have been 91.5.
  assert.notEqual(Math.round(c), 91, 'sonnet-5 must not fall back to opus DEFAULT rates');
  // A dated/suffixed variant still resolves to the sonnet-5 entry.
  const c2 = costOf('claude-sonnet-5-20260101', { input: 1_000_000, output: 0, cacheRead: 0, cacheCreate: 0 });
  assert.ok(Math.abs(c2 - 3) < 1e-6, `expected 3, got ${c2}`);
});
