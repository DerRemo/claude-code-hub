...
test('costOf: claude-opus-5 is priced at opus rates (same tier as opus-4-8)', () => {
  assert.equal(providerOf('claude-opus-5'), 'claude');
  // Opus rates (15/75/1.5) → identical to opus-4-8 for the same tokens.
  const c = costOf('claude-opus-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheCreate: 0 });
  assert.ok(Math.abs(c - 91.5) < 1e-6, `expected 91.5, got ${c}`);
});

test('costOf: claude-sonnet-5 is priced at sonnet rates, not the opus default', () => {
  assert.equal(providerOf('claude-sonnet-5'), 'claude');
  // Sonnet rates (3/15/0.3): 1M*3/1M + 1M*0.3/1M + 1M*15/1M = 18.3.
  // Regression guard: without an explicit entry it fell through to the opus
  // DEFAULT (15/75/1.5) → 91.5, ~5x too high.
  const c = costOf('claude-sonnet-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheCreate: 0 });
  assert.ok(Math.abs(c - 18.3) < 1e-6, `expected 18.3, got ${c}`);
  const cOpus = costOf('claude-opus-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheCreate: 0 });
  assert.notEqual(c, cOpus, 'sonnet-5 and opus-5 should differ in cost');
});
