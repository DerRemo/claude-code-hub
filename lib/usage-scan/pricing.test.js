...
test('costOf: claude-5 family is recognized (not silently DEFAULT)', () => {
  // claude-opus-5 is Claude Code's new default model.
  assert.equal(providerOf('claude-opus-5'), 'claude');
  assert.equal(providerOf('claude-sonnet-5'), 'claude');
  assert.equal(providerOf('claude-fable-5'), 'claude');

  // opus-5 follows the opus tier (15/75/1.5) → 91.5 for 1M/1M/1M.
  const opus5 = costOf('claude-opus-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheCreate: 0 });
  assert.ok(Math.abs(opus5 - 91.5) < 1e-6, `opus-5 expected 91.5, got ${opus5}`);

  // sonnet-5 uses the sonnet tier (3/15/0.3), distinct from the opus DEFAULT.
  const sonnet5 = costOf('claude-sonnet-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheCreate: 0 });
  assert.ok(Math.abs(sonnet5 - 18.3) < 1e-6, `sonnet-5 expected 18.3, got ${sonnet5}`);

  // fable-5 uses its Anthropic list price (10/50/1.0).
  const fable5 = costOf('claude-fable-5', { input: 1_000_000, output: 1_000_000, cacheRead: 1_000_000, cacheCreate: 0 });
  assert.ok(Math.abs(fable5 - 61) < 1e-6, `fable-5 expected 61, got ${fable5}`);

  // A dated/suffixed variant still resolves to the opus-5 entry.
  const opus5dated = costOf('claude-opus-5-20260115', { input: 1_000_000, output: 0, cacheRead: 0, cacheCreate: 0 });
  assert.ok(Math.abs(opus5dated - 15) < 1e-6, `opus-5 dated expected 15, got ${opus5dated}`);
});
