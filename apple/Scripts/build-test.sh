...
# Resolve a concrete "iPhone 17" simulator UDID on the NEWEST installed runtime.
...
# pick a Shutdown one (or the wrong runtime) and fail preflight. So we pick the
# highest installed runtime (never a hardcoded version — that breaks the moment
# it is uninstalled), boot it, and target by id for determinism. Override the
# device with PENATES_SIM_UDID, or the runtime with PENATES_SIM_RUNTIME.
RUNTIME="${PENATES_SIM_RUNTIME:-$(xcrun simctl list runtimes available \
  | grep -oE 'iOS [0-9]+\.[0-9]+' | sort -t' ' -k2 -V | tail -1)}"
