...
- **Copy & Paste in der iOS-PWA:** Die Touch-Bar über der Bildschirmtastatur hat eigene **Paste**- und **Copy**-Tasten, da es kein hardware-seitiges Cmd+C/V gibt und tmux' Mouse-Mode Textselektion sonst blockiert. **Paste** fügt die Zwischenablage per Bracketed-Paste ein. Zum Kopieren lange drücken und dann über das Terminal ziehen, um Text zu markieren (ein normaler Swipe scrollt weiterhin); Finger heben, dann **Copy** tippen, um die Auswahl in die Zwischenablage zu legen.

![Das Penates Session-Dashboard](../../../../../assets/screenshots/sessions.png)

## Grenzen

Auto-Restore gilt nur nach einem Neustart und stellt die CLI-Konversation wieder her, keinen unterbrochenen Task und keinen im RAM gespeicherten tmux-Scrollback. Fremde (nicht über den Hub gestartete) Sessions zeigen Aktivität `unknown`, bis ein Hook feuert. Die `@`-Mention-Helfer (Image-Paste, Voice) sind Claude-Code-spezifisch. Die Copy-Geste in der iOS-Touch-Bar braucht tmux, um den Drag zu empfangen, und funktioniert daher nur am Shell-Prompt — Vollbild-CLIs (Claude Code, Codex) fangen die Maus selbst ab.
