...
- **Copy/Paste auf der installierten iPhone-PWA:** Die Touch-Leiste über der Tastatur bietet **Paste** (liest die Zwischenablage beim Tap und fügt sie ins Terminal ein) und **Copy** (Long-Press, dann per Drag Text markieren; beim Loslassen wird die Auswahl aus dem tmux-Buffer geholt und beim nächsten Tap kopiert).

![Das Penates Session-Dashboard](../../../../../assets/screenshots/sessions.png)

## Grenzen

Auto-Restore gilt nur nach einem Neustart und stellt die CLI-Konversation wieder her, keinen unterbrochenen Task und keinen im RAM gespeicherten tmux-Scrollback. Fremde (nicht über den Hub gestartete) Sessions zeigen Aktivität `unknown`, bis ein Hook feuert. Die `@`-Mention-Helfer (Image-Paste, Voice) sind Claude-Code-spezifisch. Touch-Copy braucht tmux, um den Drag zu empfangen, funktioniert also am Shell-Prompt; Vollbild-CLIs (Claude Code, Codex) fangen die Maus selbst ab, die Auswahl-Geste erreicht tmux dann nicht.
