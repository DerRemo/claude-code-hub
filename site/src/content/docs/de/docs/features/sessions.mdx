...
- **Touch-Bar in der iOS-PWA:** Auf der installierten iPhone/iPad-PWA erscheint oberhalb der Software-Tastatur eine Tastenleiste mit Esc/Tab/Ctrl/Pfeilen sowie eigenen **Paste**- (liest die Zwischenablage und fügt sie via Bracketed-Paste ein) und **Copy**-Tasten (Long-Press, dann Ziehen zum Markieren, als tmux-Maus-Events weitergereicht, sodass ein normaler Swipe weiter scrollt; beim Loslassen wird die Auswahl übernommen, dann auf Copy tippen) — es gibt kein Hardware-Cmd+C/V, und der tmux-Mouse-Mode blockiert sonst die Textauswahl.
