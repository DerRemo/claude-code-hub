...
- **Kopieren & Einfügen in der iOS-PWA:** Zwei Touch-Bar-Tasten neben der Bildschirmtastatur decken ab, was ein physisches Cmd+C/V sonst leisten würde. Paste liest beim Tippen die Zwischenablage und schickt sie ins Terminal. Copy braucht Long-Press und Ziehen, um Text zu markieren (wird als Maus-Events an tmux weitergereicht, ein normales Swipe scrollt weiterhin); nach Loslassen die Copy-Taste erneut tippen, um die Auswahl in die Zwischenablage zu legen.

![Das Penates Session-Dashboard](../../../../../assets/screenshots/sessions.png)

## Grenzen

Auto-Restore gilt nur nach einem Neustart und stellt die CLI-Konversation wieder her, keinen unterbrochenen Task und keinen im RAM gespeicherten tmux-Scrollback. Fremde (nicht über den Hub gestartete) Sessions zeigen Aktivität `unknown`, bis ein Hook feuert. Die `@`-Mention-Helfer (Image-Paste, Voice) sind Claude-Code-spezifisch. Die iOS-Copy-Geste braucht tmux selbst als Empfänger des Drags, funktioniert also an einem Shell-Prompt; Vollbild-CLI-UIs (Claude Code, Codex) fangen die Maus selbst ab und bieten keine kopierbare Auswahl.
