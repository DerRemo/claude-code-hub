...
- **Copy/Paste auf dem Smartphone:** Auf der installierten iOS-PWA bietet die Touch-Bar des Terminals **Paste** (liest die Zwischenablage und fügt sie per Bracketed-Paste ein) und **Copy** (Long-Press, dann per Drag Text selektieren — als tmux-Maus-Events weitergereicht, sodass ein normaler Swipe weiter scrollt; beim Loslassen wird die Selektion geladen, der nächste Copy-Tap legt sie in die Zwischenablage).

![Das Penates Session-Dashboard](../../../../../assets/screenshots/sessions.png)

## Grenzen

Auto-Restore gilt nur nach einem Neustart und stellt die CLI-Konversation wieder her, keinen unterbrochenen Task und keinen im RAM gespeicherten tmux-Scrollback. Fremde (nicht über den Hub gestartete) Sessions zeigen Aktivität `unknown`, bis ein Hook feuert. Die `@`-Mention-Helfer (Image-Paste, Voice) sind Claude-Code-spezifisch. Copy auf dem Smartphone braucht tmux, um den Drag zu empfangen, funktioniert also an einem Shell-Prompt; Full-Screen-CLIs (Claude Code, Codex) fangen die Maus selbst ab.
