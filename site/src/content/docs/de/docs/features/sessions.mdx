...
- **Copy/Paste in der iOS-PWA:** Die installierte iPhone-App bekommt zwei Touch-Bar-Tasten, da es kein Hardware-Cmd+C/V gibt. **Paste** liest die Zwischenablage beim Tap und sendet sie per Bracketed-Paste. **Copy** startet mit Long-Press-und-Drag (als Mouse-Events an tmux weitergereicht, ein normaler Swipe scrollt weiterhin); beim Loslassen wird die Selektion aus dem tmux-Paste-Buffer gelesen und beim nächsten Copy-Tap kopiert. Copy braucht tmux als Empfänger des Drags, funktioniert also am Shell-Prompt; Vollbild-CLIs, die die Maus selbst abfangen (Claude Code, Codex), sind von Copy nicht betroffen.
