...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS Option-als-Meta (standardmäßig aus, damit ⌥ auf EU-Tastaturen native Zeichen wie `@ | { } [ ] ~` zusammensetzt; einschalten, um ⌥ stattdessen für die Readline-Wortnavigation zu nutzen).
