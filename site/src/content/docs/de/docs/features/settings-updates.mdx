...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS ⌥ als Meta (standardmäßig aus, sodass Option Layout-Zeichen wie `@ | { }` auf nicht-US-Tastaturen tippt; einschalten für Meta-Kürzel wie ⌥B/⌥F zur Wort-Navigation).
