...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und die Terminal-Bell. Der Schalter **macOS: ⌥ as Meta** ist standardmäßig aus, damit Option auf EU-Tastaturen native Layout-Zeichen komponiert (`@ | { }`); eingeschaltet sendet Option stattdessen Meta-Shortcuts (⌥B/⌥F Wort-Navigation). Änderungen wirken sofort auf das laufende Terminal.
