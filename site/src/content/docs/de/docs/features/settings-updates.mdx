...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS-Option-als-Meta (standardmäßig aus, damit Option auf nicht-US-Tastaturen Layout-Zeichen wie `@ | { }` tippt; an sendet Meta-Kürzel für die Wort-Navigation in readline).
