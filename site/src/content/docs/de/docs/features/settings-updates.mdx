...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und Option-als-Meta (nur macOS, standardmäßig aus, damit von EU-Tastaturen komponierte Zeichen wie `@`/`|`/`{`/`}` normal funktionieren; einschalten, um Option stattdessen für Readline-Wortnavigation zu nutzen).
