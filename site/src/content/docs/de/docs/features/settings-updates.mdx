...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und (macOS) ob ⌥ (Option) als Meta für die Readline-Wort-Navigation wirkt — standardmäßig aus, damit Option auf nicht-US-Tastaturen weiterhin Layout-Zeichen wie `@ | { } [ ] ~` tippt.
