...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS Option-als-Meta (standardmäßig aus, damit die Wahltaste native Zeichen wie `@ | { }` auf nicht-US-Tastaturen tippt; an sendet Meta-Kürzel wie ⌥B/⌥F zur Wort-Navigation).
