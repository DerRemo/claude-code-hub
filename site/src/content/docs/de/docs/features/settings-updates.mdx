...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS Option-als-Meta (standardmäßig aus, sodass Option auf nicht-US-Tastaturen native Zeichen wie `@ | { }` tippt; einschalten sendet stattdessen Meta-Kürzel wie `⌥B`/`⌥F` für die Wort-Navigation in readline).
