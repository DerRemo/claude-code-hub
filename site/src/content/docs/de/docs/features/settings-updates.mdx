...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS ⌥ als Meta (standardmäßig aus, sodass die Wahltaste Layout-Zeichen wie `@ | { }` auf nicht-US-Tastaturen tippt; einschalten für ⌥B/⌥F-Wort-Navigation-Kürzel).
