...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS ⌥-als-Meta (standardmäßig aus, damit Option auf nicht-US-Tastaturen native Zeichen wie `@ | { }` tippt; einschalten für Meta-Kürzel zur Wort-Navigation in readline).
