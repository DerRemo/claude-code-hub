...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und **macOS: ⌥ als Meta** (standardmäßig aus — aus tippt Layout-Zeichen wie deutsches `@`/`|`/`{`/`}`, die auf EU-Tastaturen hinter Option liegen; an sendet stattdessen Readline-Wortnavigation wie ⌥B/⌥F).
