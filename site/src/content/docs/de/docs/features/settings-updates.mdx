...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und (nur macOS) ob die Option-Taste als Meta wirkt — standardmäßig aus, damit sie Layout-Zeichen tippt (`@ | { } [ ] ~` auf Nicht-US-Tastaturen) statt Meta-Shortcuts wie ⌥B/⌥F für die Wort-Navigation zu senden.
