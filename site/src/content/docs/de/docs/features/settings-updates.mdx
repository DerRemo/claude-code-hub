...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select sowie (macOS-Clients) **⌥ als Meta** — standardmäßig aus, sodass die Wahltaste Layout-Zeichen tippt (`@ | { }` auf nicht-US-Tastaturen); aktiviert sendet Option stattdessen Meta-Kürzel (⌥B/⌥F Wort-Navigation).
