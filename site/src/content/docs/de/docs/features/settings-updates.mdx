...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und macOS-Option-als-Meta. Letzteres ist standardmäßig aus, damit Option native Zeichen tippt (`@ | { }` auf EU-Layouts); einschalten, um stattdessen Meta-Shortcuts zu senden (⌥B/⌥F Wortnavigation).
