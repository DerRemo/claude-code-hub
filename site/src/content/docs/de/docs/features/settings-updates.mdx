...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und (nur macOS) ob ⌥ Option als Meta wirkt.
...
**macOS: ⌥ als Meta:** standardmäßig aus, sodass Option die Layout-Zeichen tippt, die nicht-US-Tastaturen dahinter legen (Deutsch: `@` = Option+L, dazu `|` `\` `{` `}` `[` `]` `~`). Einschalten sendet stattdessen Meta-Kürzel (⌥B/⌥F Wort-Navigation) — der Readline-Tradeoff. Wirkt sofort auf das laufende Terminal.
