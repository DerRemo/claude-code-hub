...
- **Terminal:** Schriftgröße, Scrollback-Zeilen, Mouse-Mode (`TMUX_MOUSE`), Copy-on-Select und (macOS) ob ⌥ als Meta wirkt.
...
**macOS: ⌥ als Meta:** Standardmäßig aus, damit die Wahltaste native Zeichen tippt (`@ | { } [ ] ~` auf europäischen Tastaturlayouts); einschalten, damit Option stattdessen Meta-Kürzel sendet (⌥B/⌥F für Wort-Navigation in readline). Wirkt sofort auf das laufende Terminal.
