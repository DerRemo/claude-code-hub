...
- **iOS PWA copy/paste:** the installed iPhone app adds two touch-bar keys since there's no hardware Cmd+C/V. **Paste** reads the clipboard on tap and sends it with bracketed-paste. **Copy** starts on a long-press-then-drag (forwarded to tmux as mouse events, so a normal swipe still scrolls); on release the selection is read back from the tmux paste buffer and copied on the next Copy tap. Copy needs tmux to receive the drag, so it works at a shell prompt; full-screen CLIs that capture the mouse themselves (Claude Code, Codex) are unaffected by Copy.
