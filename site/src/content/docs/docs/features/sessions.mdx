...
- **Copy/paste on the installed iOS PWA:** the touch-bar above the keyboard adds **Paste** and **Copy** keys, since there's no hardware Cmd+C/V and tmux mouse-mode blocks normal text selection. Tap **Paste** to insert the clipboard as a bracketed paste. To copy, long-press then drag to select text (a swipe without a long-press still scrolls); release to stage the selection, then tap **Copy**.
