...
- **iOS PWA touch-bar:** on the installed iPhone/iPad PWA, a keyboard accessory bar appears above the software keyboard with Esc/Tab/Ctrl/arrows plus dedicated **Paste** (reads the clipboard and pastes with bracketed-paste) and **Copy** (long-press then drag to select text, forwarded as tmux mouse events so a normal swipe still scrolls; release the selection, then tap Copy) keys — there's no hardware Cmd+C/V and tmux mouse-mode otherwise blocks text selection.
