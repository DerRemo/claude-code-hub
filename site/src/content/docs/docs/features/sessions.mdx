...
- **Copy/paste on the iOS PWA:** the touch keyboard accessory adds Paste (reads the clipboard in the tap gesture and pastes with bracketed-paste) and Copy (long-press then drag to select — forwarded to tmux as mouse events so a normal swipe still scrolls; release copies the selection on the next Copy tap). Copy needs tmux to receive the drag, so it works at a shell prompt; full-screen CLIs like Claude Code or Codex capture the mouse themselves.
