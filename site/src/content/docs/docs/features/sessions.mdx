...
- **Copy/paste on the iOS PWA:** the touch-bar above the keyboard adds Copy and Paste keys, since tmux mouse-mode blocks native text selection and there is no hardware Cmd+C/V. Paste reads the clipboard on tap; long-press then drag selects text (a normal swipe still scrolls), and the selection copies on the next Copy tap.
