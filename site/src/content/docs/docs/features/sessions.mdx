...
- **iOS copy/paste:** the installed iPhone PWA has no hardware ⌘C/⌘V, and tmux mouse mode blocks native text selection, so the touch-bar above the keyboard adds **Paste** (reads the clipboard and pastes into the terminal) and **Copy** (long-press then drag to select, then tap Copy to fetch the selection from tmux and copy it).
