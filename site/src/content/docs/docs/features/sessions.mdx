...
- **Copy/paste on the iOS PWA:** the touch-bar adds dedicated **Copy** and **Paste** keys, since there's no hardware shortcut and tmux mouse-mode blocks normal text selection. Paste reads the clipboard on tap; long-press then drag selects text (a swipe still scrolls), and Copy grabs that selection from the tmux buffer. Copy needs tmux to see the drag, so it works at a shell prompt — full-screen CLIs like Claude Code or Codex capture the mouse themselves.
