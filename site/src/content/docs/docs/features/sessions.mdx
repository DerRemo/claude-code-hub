...
- **Copy/paste on the iOS PWA:** the installed iPhone app has no hardware Cmd+C/V, and tmux mouse-mode blocks native text selection, so the touch-bar adds two keys. **Paste** reads the clipboard on tap (iOS shows its paste prompt) and sends it with bracketed-paste. **Copy** — long-press then drag to select (a plain swipe still scrolls); release fetches the selection from the tmux buffer, and the next Copy tap copies it. Selection needs tmux to receive the drag, so it works at a shell prompt; full-screen CLIs like Claude Code and Codex capture the mouse themselves.
