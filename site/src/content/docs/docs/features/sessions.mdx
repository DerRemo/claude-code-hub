...
- **Copy/paste on the iOS PWA:** since tmux mouse-mode blocks native text selection and there is no hardware Cmd+C/V, the touch-bar adds dedicated Paste (reads the clipboard on tap) and Copy (long-press then drag to select, release to copy) keys.
