...
- **Copy/paste on the installed iPhone/iPad PWA:** the terminal's touch-bar adds dedicated **Copy** and **Paste** keys, since tmux mouse-mode blocks native text selection. Long-press then drag to select text (a normal swipe still scrolls); tap **Copy** to grab the selection from the tmux buffer, tap **Paste** to insert the clipboard with bracketed-paste.
