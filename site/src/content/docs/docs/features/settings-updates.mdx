...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and the terminal bell. The **macOS: ⌥ as Meta** toggle is off by default so Option composes native layout characters (`@ | { }` on EU keyboards); switch it on to send Meta shortcuts instead (⌥B/⌥F word navigation). Changes apply live to the running terminal.
