...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and (macOS only) whether ⌥ (Option) acts as Meta. Off by default so Option still types layout characters (`@ | { }` on non-US keyboards); switch it on to get Meta-based word navigation (⌥B/⌥F) instead.
