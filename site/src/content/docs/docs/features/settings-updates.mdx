...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and **macOS: ⌥ as Meta** (default off — off types layout characters like German `@`/`|`/`{`/`}` that live behind Option on EU keyboards; on sends readline word-navigation shortcuts like ⌥B/⌥F instead).
