...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and (macOS only) whether the Option key acts as Meta — off by default so it types layout characters (`@ | { } [ ] ~` on non-US keyboards) instead of sending Meta shortcuts like ⌥B/⌥F word navigation.
