...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and (macOS) whether ⌥ acts as Meta.
...
**macOS: ⌥ as Meta:** off by default so Option composes native characters (`@ | { } [ ] ~` on European keyboard layouts); switch it on to have Option send Meta shortcuts instead (⌥B/⌥F for readline word navigation). Applies live to the running terminal.
