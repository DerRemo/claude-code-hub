...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and macOS ⌥-as-Meta (off by default, so Option types layout characters like `@ | { }` on non-US keyboards; enable it to get ⌥B/⌥F word-navigation shortcuts instead).
