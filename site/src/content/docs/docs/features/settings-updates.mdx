...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and (macOS clients) **⌥ as Meta** — off by default so Option types layout characters (`@ | { }` on non-US keyboards); on, Option sends Meta shortcuts (⌥B/⌥F word navigation) instead.
