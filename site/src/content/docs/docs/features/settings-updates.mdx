...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and macOS Option-as-Meta (off by default, so ⌥ composes native characters like `@ | { } [ ] ~` on EU keyboards; switch it on to use ⌥ for readline word navigation instead).
