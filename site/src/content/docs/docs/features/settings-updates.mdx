...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and (macOS) whether ⌥ (Option) acts as Meta for readline word navigation — off by default so Option still types layout characters like `@ | { } [ ] ~` on non-US keyboards.
