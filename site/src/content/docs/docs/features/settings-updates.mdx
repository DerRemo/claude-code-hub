...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and Option-as-Meta (macOS only, off by default so EU-keyboard composed characters like `@`/`|`/`{`/`}` type normally; turn it on to use Option for readline word navigation instead).
