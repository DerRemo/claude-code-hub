...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and macOS ⌥-as-Meta (off by default, so Option composes native characters like `@ | { }` on non-US keyboards; switch it on for readline word-navigation shortcuts instead).
