...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and macOS Option-as-Meta (off by default, so Option composes native characters like `@ | { }` on non-US keyboards; turn it on to send Meta shortcuts such as `⌥B`/`⌥F` for readline word navigation).
