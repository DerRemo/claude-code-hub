...
- **Terminal:** font size, scrollback lines, mouse mode (`TMUX_MOUSE`), copy-on-select, and macOS Option-as-Meta. The latter is off by default so Option composes native characters (`@ | { }` on EU keyboards); switch it on to send Meta shortcuts instead (⌥B/⌥F word navigation).
